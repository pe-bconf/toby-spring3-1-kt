...
class UserServiceTest {
    ...
    fun upgradeLevels() {
        ...
        val mockUserDao = mock<UserDao>(UserDao::class.java)
        ...
        assertThat(users[3].level, `is`(Level.GOLD))

        // 함수의 파라미터 캡처를 위한 오브젝트
        val mailMessageArg = ArgumentCaptor.forClass(SimpleMailMessage::class.java)
        // 캡처한 오브젝트가 2번 불리우고, send() 에 전송된 오브젝트들 캡처
        verify(mockMailSender, times(2)).send(mailMessageArg.capture())
        val mailMessages = mailMessageArg.allValues
        assertThat(mailMessages[0].to[0], `is`(users[1].email))
        assertThat(mailMessages[1].to[0], `is`(users[3].email))
    }
    ...
    private fun checkLevelUpgraded(updated: User, expectedId: String, expectedLevel: Level) {
        assertThat(updated.id, `is`(expectedId))
        assertThat(updated.level, `is`(expectedLevel))
    }
    ...
    fun upgradeAllOrNothing() {
        ...
        txHandler.setTransactionManger(this.transactionManager)
        txHandler.setPattern("upgradeLevels")

        val txUserService = Proxy.newProxyInstance(
            UserService::class.java.classLoader,
            arrayOf<Class<*>>(UserService::class.java),
            txHandler
            ) as UserService
        ...
        checkLevelUpgraded(users[1], "joytouch", Level.BASIC)
    }
