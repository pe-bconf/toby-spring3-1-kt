...
class UserDaoJdbc: UserDao {
    ...
    override fun get(id: String?): User {
        return this.jdbcTemplate.queryForObject("select id, name, password, level, login, recommend from users where id = ?", arrayOf(id),
            userMapper)
    }
    ...
    override fun update(user1: User): Int {
        return this.jdbcTemplate.update(
            ...
            user1.name, user1.password, user1.level?.intValue(), user1.login, user1.recommend, user1.id
        )
    }
}